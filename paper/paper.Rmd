...
gs_football <- readr::read_rds(here::here("data",
                           "gs-football.rds"))
```

```{r mismatch}
# Now we bring the third and final review 
gs_mismatches <- readr::read_csv(here::here("data",
"missing-data-mismatches.csv"))
...
independently extracted the data (`r md_reported` agreement). Results are reported
as the proportion and 95% confidence interval. (NOTE: need to get the final third reviewer part in)

# Results

```{r}
# NOTE: update with the final third reviewer
# need to update the gs_football
library(binom)
# !!comment to explain NAs
successes <- sum(gs_football$any_missing_data_statement == 1, na.rm = TRUE)
trials <- sum(!is.na(gs_football$any_missing_data_statement))
missing_binom_conf <- binom.confint(x = successes, 
              n = trials, 
              conf.level = 0.95, 
              methods = "exact")

n_screened <- n_distinct(gs_football$key)
miss_reported_pct <- percent(missing_binom_conf$mean, accuracy = 0.1)
miss_reported_pct_lower <- percent(missing_binom_conf$lower, accuracy = 0.1)
miss_reported_pct_upper <- percent(missing_binom_conf$upper, accuracy = 0.1)

n_shared <- sum(gs_football$was_data_shared == 1)
```


```{r}
# now of the articles that don't mention missingness
gs_football_no_mention_sharing <- gs_football %>% 
  filter(any_missing_data_statement == 0)

n_no_mention_missings <- n_distinct(gs_football_no_mention_sharing$key)

no_mention_successes <- sum(gs_football_no_mention_sharing$was_data_shared == 1, 
                            na.rm = TRUE)
no_mention_trials <- sum(!is.na(gs_football_no_mention_sharing$was_data_shared))
n_mention_missing_binom_conf <- binom.confint(x = no_mention_successes, 
                                              n = no_mention_trials, 
                                              conf.level = 0.95, 
                                              methods = "exact")

percent_binom_conf <- function(binom_conf){
  mean_pct <- percent(n_mention_missing_binom_conf$mean, accuracy = 0.1)
lower_pct <- percent(n_mention_missing_binom_conf$lower, accuracy = 0.1)
upper_pct <- percent(n_mention_missing_binom_conf$upper, accuracy = 0.1)

return(c(mean_pct = mean_pct,
         lower_pct = lower_pct,
         upper_pct = upper_pct))
}

percent_binom_conf(n_mention_missing_binom_conf)

```


Of the 199 articles screened, `r n_screened` met the inclusion criteria, representing
\~10% of the articles found (see Supplement 2, Figure 1). The proportion of
articles that reported missing data was `r miss_reported_pct` (95% confidence interval;
`r miss_reported_pct_lower` to `r miss_reported_pct_upper`) or `r successes`/`r n_screened`. 

Of the articles that did not mention missing data `n_no_mention_missings` / `r n_screened`, 7.1% (3.3% to 13.1%) shared their dataset, making
...
# write_rds(d, here::here("data", "afl-data.rds"))
d <- readr::read_rds(here::here("data", "afl-data.rds"))
...
sprint_distance <- truncnorm::rtruncnorm(n = nrow(afl_data), 
                                         a = 0, 
                                         b = Inf, 
                                         mean = 2000, 
...
aq_vis_miss <- vis_miss(data_plot_clean) + labs(tag = "(A)")
  #theme(text = element_text(size = 14))

aq_fct <- gg_miss_fct(data_plot_clean, team) + 
  #theme(text = element_text(size = 14)) +
  labs(x = "Team", y = "Variable", tag = "(B)") +
  theme(axis.text.x = element_text(size = 5))

gg_miss_scatter <- ggplot(data_plot_clean,
       aes(x = sprint_distance,
           y = sprints)) +
...
aq_miss_var <- gg_miss_var(data_plot_clean) + labs(tag = "(D)")
```
