...
pattern of missingness--determine how to handle missings on a
...
A missing value should have been observed or measured in the data, but was not. For example, a goal scored in football takes place, but the scorekeeper wasn't paying attention and did not record an event. This "goal" is now a missing value. Compare this to a game being rained out and no game taking place. There is no missing data because there is no data to be observed. A final example - a study screens out individuals who do not meet criteria for the study. These are not missing values, because there was no intent for these to be observed or measured.
...
sports science, researchers seldom consider missing data [@Schafer2002] (<< NOTE - I'm not sure we mean that?).
