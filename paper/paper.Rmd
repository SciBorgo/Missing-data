...
aq_fct <- gg_miss_fct(data_plot, team) + 
  #theme(text = element_text(size = 14)) +
  labs(x = "Team", y = "Variable")

gg_miss_scatter <- ggplot(data_plot,
       aes(x = sprint_distance,
           y = sprints)) +
  geom_miss_point(size = 2) + 
  theme_minimal() +
  #theme(text = element_text(size = 14)) +
  labs(x = "Total sprint distance (m)", y = "Number of sprints (count)", colour = "Missing")

aq_miss_var <- gg_miss_var(data_plot)

# Grid plots
cowplot::plot_grid(aq_vis_miss, aq_fct, gg_miss_scatter, aq_miss_var, 
                   nrow = 2,
                   ncol = 2,
                   align = "hv",
                   axis = "tblr",
