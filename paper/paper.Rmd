...
For a value to be missing, it should have been observed in the data, but was not. For example, a goal scored in football takes place, but the scorekeeper wasn't paying attention and did not record an event. This "goal" is now a missing value. Compare this to a game being rained out and no game taking place. There is no data to be observed, so there is no missing data. 
