...
Affiliations: 0000-0003-1460-8722
...
studied. A survey of `r n_screened` articles, sampled at random, was conducted to
...
statement was low, at only `r binom_ci_miss_reported$mean_pct`(95% CI; `r binom_ci_miss_reported$lower_pct` to `r binom_ci_miss_reported$upper_pct`); suggesting that 
...
independently extracted the data (`r md_reported` agreement). Disagreements were resolved by author DNB. Results are reported as the proportion and 95% confidence interval (CI).
...
articles that reported missing data was `r binom_ci_miss_reported$mean_pct`(95% CI; `r binom_ci_miss_reported$lower_pct` to `r binom_ci_miss_reported$upper_pct`).


Of the articles that did not mention missing data `r n_no_mention_missings` / `r n_screened`, `r binom_ci_data_shared$mean_pct`(95% CI; `r binom_ci_data_shared$lower_pct` to `r binom_ci_data_shared$upper_pct`). shared their dataset, making
...
Our survey of `r n_screened` articles published on football related topics found
that only one in 13 papers (`r binom_ci_miss_reported$mean_pct`) reported missing data. It is reasonable
