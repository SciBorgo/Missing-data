...
observations with mean values (e.g., participant or group average) [@refs](https://doi.org/10.1016/j.jsams.2018.05.019, https://doi.org/10.1016/j.jsams.2008.01.009). 
...
perform a sensitivity analysis (**ref circulation article-Nick, possible reference from dave - https://doi.org/10.1191%2F1740774504cn032oa**). This is
discussed below.
...
data_afl <- d %>% select(ID, Round, Playing.for, Venue)

# Simulate injury and sprint variables
injury <- rbinom(n = nrow(d), 1, prob = 0.1)
sprints <- rpois(n = nrow(d), lambda = 100)
sprint_distance <- truncnorm::rtruncnorm(n = nrow(d), a = 0, b = Inf, mean = 2000, sd = 500) # Check if this is reasonable (mean 2 km per game)

data <- cbind(injury, sprints, sprint_distance)

# Impute missing values
data_impute <- ampute(data = data[,1:3])
data_with_missing <- data_impute$amp

# Put the dataset together
data_plot <- cbind(data_afl, data_with_missing)
...
```{r missing-overview, fig.cap = "Overviews of missing values in simulated injury data in Australian Football matches for August, 2020. Panel A shows... Panel B shows ... Panel C shows ...", fig.height = 10}

aq_vis_miss <- vis_miss(data_plot)

aq_upset <- gg_miss_upset(data_plot)

aq_fct <- gg_miss_fct(data_plot, team)

aq_miss_var <- gg_miss_var(data_plot) + labs(title = "A")
aq_miss_case <- gg_miss_case(data_plot) + labs(title = "B")
aq_vis_miss <- vis_miss(data_plot) + labs(title = "C")
...
For discussion on imputing the response value, see ... [@refs]. **(note: take a look again - this is the discusson of missing y values section - and possible reference relevant to RCT's is this: https://doi.org/10.1191%2F1740774504cn032oa)**
...
with only about one in 20 studies reporting missing data.
...
data could suggest that this aspect of data anlaysis receives little attention. 
